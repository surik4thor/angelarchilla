export async function getDreamHistory() {
  const res = await api.get('/dreams/history');
...
export async function fetchHoroscope(fechaNac, genero) {
  ...
  const res = await api.get(`/horoscope?fechaNac=${fechaNac}&genero=${genero}`);
...
export async function getReadingStatsAndHistory() {
  ...
  const statsPromise = api.get('/auth/me');
  const historyPromise = api.get('/readings/history');
